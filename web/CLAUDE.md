...
## Panel UI Ordering Conventions

Within each algorithm panel, UI elements must appear in this order:

1. **Prerequisite selectors** (e.g. algorithm dropdown) — controls that determine which other options are shown, placed first
2. **Configuration options** (checkboxes, numeric inputs) — general settings grouped in `nhg-options` or `maxent-params` sections
3. **Text/file inputs** (e.g. "A priori rankings") — near the bottom, above action buttons
4. **Action buttons** (Run, Download, Reset) — always last, in the `action-bar` div

Section sub-headers (`.nhg-options-label`) must:
- Use sentence case (only initial capitalization): e.g. "Ranking argumentation", not "Ranking Argumentation"
- Omit trailing colons: e.g. "Prior", not "Prior:"
